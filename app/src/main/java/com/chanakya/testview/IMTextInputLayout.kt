...
class IMTextInputLayout @JvmOverloads constructor(
    context: Context,
    ...
    private val hintPaint = Paint(ANTI_ALIAS_FLAG).apply {
        color = Color.GRAY
        style = Paint.Style.STROKE
        strokeWidth = 4f
        textSize = collapsedTextSize
    ...
    private fun initAttrs(attributeSet: AttributeSet) {
        with(
            ...
            setHintColor( getColor(
                R.styleable.AwesomeTextInputLayout_hintColor,
                hintPaint.color
            ))
            setBorderStroke( getDimension(
    ...
    fun setCornerRadius(dimension: Float) {
        cornerRadius = dimension
    }

    fun setBorderStroke(dimension: Float) {
    ...
    fun updateHint(newHint: String) {
        this.hint = newHint
        editText?.hint = ""  // Clear the original hint from the EditText
